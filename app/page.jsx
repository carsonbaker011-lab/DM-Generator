...
export default function Home() {
  ...
  const [tab, setTab] = useState('generator'); // 'generator' | 'leads'
    ...
    <>
      ...
      <div className="app">
        ...
        <div className="tab-bar">
          <button className={`tab-btn${tab==='generator'?' active':''}`} onClick={()=>setTab('generator')}>
            ✦ DM Generator
          </button>
        ...
        {tab === 'leads' ? <LocalLeads usageData={usageData} onUpgrade={()=>setShowPricing(true)} /> : (<>
...
// ── Local Leads Component ─────────────────────────────
function LocalLeads({ usageData, onUpgrade }) {
  const [city, setCity]       = useState('');
  const [bizType, setBizType] = useState('');
  const [loading, setLoading] = useState(false);
  const [results, setResults] = useState([]);
  const [error, setError]     = useState('');
  const [scripts, setScripts] = useState({}); // place_id -> script sections
  const [genLoading, setGenLoading] = useState({}); // place_id -> bool

  const BIZ_TYPES = [
    'Plumbers', 'Electricians', 'HVAC companies', 'Roofers', 'Landscapers',
    'Auto repair shops', 'Restaurants', 'Hair salons', 'Dentists', 'Gyms',
    'Lawyers', 'Accountants', 'Real estate agents', 'Photographers', 'Chiropractors',
  ];

  async function search() {
    if (!city.trim() || !bizType.trim()) { setError('Please enter a city and business type.'); return; }
    setError(''); setLoading(true); setResults([]); setScripts({});
    try {
      const res = await fetch('/api/places', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ city: city.trim(), type: bizType.trim() }),
      });
      const data = await res.json();
      if (!res.ok) { setError(data.error || 'Search failed.'); return; }
      if (data.results.length === 0) setError('No results found. Try a different city or business type.');
      else setResults(data.results);
    } catch { setError('Connection error. Please try again.'); }
    finally { setLoading(false); }
  }

  async function generateScript(biz) {
    setGenLoading(g => ({ ...g, [biz.place_id]: true }));
    const prompt = `You are an elite sales coach. Write a cold call script for reaching out to a local business with no website.
...
- They have NO website — this is the pain point to focus on

Goal: Sell them a website or digital marketing services. Be specific about their business name and location.
Keep it natural, conversational, under 90 seconds to deliver.

Respond ONLY with valid JSON. No markdown, no backticks:
{"opener":"...","value_pitch":"...","objection":"...","close":"..."}`;

    const raw = await callAI(prompt);
    try {
      const parsed = JSON.parse((raw||'').replace(/```json|```/g,'').trim());
      setScripts(s => ({ ...s, [biz.place_id]: parsed }));
    } catch { setError('Could not generate script. Try again.'); }
    setGenLoading(g => ({ ...g, [biz.place_id]: false }));
  }

  return (
    <div className="leads-wrap">
      <div className="card" style={{marginBottom:16}}>
        <p className="leads-desc">Find local businesses with great reviews but no website — perfect cold call prospects.</p>
        <div className="leads-form">
          <div className="leads-field">
            <label className="form-label">City</label>
            <input placeholder="e.g. Denver, Austin, Miami..." value={city} onChange={e=>setCity(e.target.value)}
              onKeyDown={e=>e.key==='Enter'&&search()} />
          </div>
          <div className="leads-field">
            <label className="form-label">Business Type</label>
            <input list="biz-types" placeholder="e.g. Plumbers, Dentists..." value={bizType} onChange={e=>setBizType(e.target.value)}
              onKeyDown={e=>e.key==='Enter'&&search()} />
            <datalist id="biz-types">
              {BIZ_TYPES.map(t => <option key={t} value={t} />)}
            </datalist>
          </div>
          <button className="btn-gen" style={{marginTop:0}} onClick={search} disabled={loading}>
      ...
      {results.length > 0 && (
        <div className="leads-results">
          <div className="leads-header">
            <span className="output-title">✦ {results.length} Leads Found — No Website, High Reviews</span>
          </div>
          {results.map(biz => (
            <div key={biz.place_id} className="lead-card">
              <div className="lead-top">
                <div className="lead-info">
                  <div className="lead-name">{biz.name}</div>
                  <div className="lead-meta">
                    <span className="lead-rating">⭐ {biz.rating} ({biz.reviews?.toLocaleString()} reviews)</span>
                    <span className="lead-sep">·</span>
                    <span className="lead-address">{biz.address}</span>
                  </div>
                  {biz.phone && <div className="lead-phone">📞 {biz.phone}</div>}
                  {biz.maps_url && <a className="lead-link" href={biz.maps_url} target="_blank" rel="noopener noreferrer">View on Maps →</a>}
                </div>
                <div className="lead-badge">No Website</div>
              </div>

              {!scripts[biz.place_id] ? (
                <button className="btn-gen-script" onClick={()=>generateScript(biz)} disabled={genLoading[biz.place_id]}>
                  {genLoading[biz.place_id] ? <><span className="spinner" />Generating…</> : '📞 Generate Cold Call Script'}
                </button>
              ) : (
                <LeadScript script={scripts[biz.place_id]} biz={biz} onRegen={()=>generateScript(biz)} loading={genLoading[biz.place_id]} />
              )}
            </div>
          ))}
        </div>
...
function LeadScript({ script, biz, onRegen, loading }) {
  const [copied, setCopied] = useState(false);
  const secLabels = { opener:'Opener', value_pitch:'Value Pitch', objection:'Handle Objection', close:'Close' };
  const secClass  = { opener:'', value_pitch:'', objection:'call-obj', close:'call-cls' };

  function copyAll() {
    const text = Object.entries(script).map(([k,v])=>`[${k.toUpperCase()}]\n${v}`).join('\n\n');
    navigator.clipboard.writeText(text).then(()=>{ setCopied(true); setTimeout(()=>setCopied(false),2000); });
  }

  return (
    <div className="lead-script">
      <div className="lead-script-top">
        <span className="lead-script-label">📞 Cold Call Script for {biz.name}</span>
        <div style={{display:'flex',gap:6}}>
          <button className={`btn-action${copied?' btn-copied':''}`} onClick={copyAll}>{copied?'✓ Copied':'Copy Script'}</button>
          <button className="btn-action" onClick={onRegen} disabled={loading}>Regenerate</button>
        </div>
      </div>
      {Object.entries(script).map(([key,val])=>(
        <div key={key} className="call-sec">
          <div className="call-sec-label">{secLabels[key]||key}</div>
          <div className={`call-sec-text${secClass[key]?' '+secClass[key]:''}`}>{val}</div>
        </div>
      ))}
    </div>
